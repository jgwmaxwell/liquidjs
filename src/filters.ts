...
const escape = (str: any) =>
  stringify(str).replace(/&|<|>|"|'/g, m => escapeMap[m]);
...
const getMaxFixed = (l: number, r: number) =>
  Math.max(getFixed(l), getFixed(r));

const bindFixed = (cb: (l: number, r: number) => number) =>
  (l: number, r: number) =>
